...
# Import our modules
from gan_evaluation_metrics import count_einsum_operations
...
def quick_test(generator_path='zk_conditional_gan_v2_final.pth'):
    """Quick test to check if model is training properly"""
    print("="*70)
    ...
    with torch.no_grad():
        ...
        labels = torch.arange(10).to(device)  # One of each class

        fake_images = generator(noise, labels)

        # Check basic statistics
        mean = fake_images.mean().item()
        std = fake_images.std().item()
        min_val = fake_images.min().item()
        max_val = fake_images.max().item()
        ...
        # Check diversity
        diversity = fake_images.std(dim=0).mean().item()
        print(f"  Diversity: {diversity:.4f}")

        if diversity < 0.05:
            print("⚠️ WARNING: Low diversity - possible mode collapse!")
        elif diversity > 0.8:
            print("⚠️ WARNING: High diversity - model might not be learning!")
        else:
            print("✓ Diversity looks healthy")

        if abs(mean) > 0.5:
            print("⚠️ WARNING: Mean shifted from zero")

        if std < 0.3:
            print("⚠️ WARNING: Low standard deviation")
        elif std > 1.5:
            print("⚠️ WARNING: High standard deviation")
...
if __name__ == '__main__':
    ...
    if args.quick:
        quick_test(args.generator_path)
    else:
